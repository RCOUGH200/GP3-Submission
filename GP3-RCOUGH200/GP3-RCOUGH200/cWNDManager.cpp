...
LRESULT CALLBACK cWNDManager::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		if (wParam == 'p' || wParam == 'P')  //If p or P was pressed
		{
			//When on main menu or instructions
			if (menuNum==0 || menuNum == 2)
			menuNum = 1; //Go to main game menu
		}
		if (wParam == 't' || wParam == 'T')  //If t or T was pressed
		{
			menuNum = 2; //Go to instructions menu
		}
		if (wParam == 'q' || wParam == 'Q')  //If q or Q was pressed
		{
			//Check the button is not still being held still
			if (!holdingCamTog){
				holdingCamTog = true;
				//If the camera is in top down third person and q is pressed
				if (camToggled)
					//Switch to first person
					camToggled = false;
				//If already in first person, switch back to third
				else
					//Switch to third person
					camToggled = true;
			}
		}
		if (wParam == 'm' || wParam == 'M')  //If m or M was pressed
		{
			//Make sure M was stopped being held
			if (!holdingMute){
				holdingMute = true; //M is being held
	...
	case WM_KEYUP:
		...
		if (wParam == VK_LEFT) //If the Left Arrow key was pressed
		{
			xDir = 0; //No longer going right
		}
		if (wParam == VK_RIGHT) //If the Left Arrow key was pressed
		{
			xDir = 0; //No longer going left
		}
		if (wParam == VK_UP) //If the UP Arrow key was pressed
		{
			zDir = 0; //No longer going forward
		}
		if (wParam == VK_DOWN) //If the Down Arrow key was pressed
		{
			zDir = 0; //No longer going back
		}
		if (wParam == VK_SPACE) //If the Down Arrow key was pressed
		{
			fire = false; //No longer firing
		}
		if (wParam == 'm' || wParam == 'M')  //If m or M was stopped being held
		{
			holdingMute = false; //M is not being held
		}
		if (wParam == 'q' || wParam == 'Q')  //If q or Q was stopped being held
		{
			holdingCamTog = false; //Cam toggle is not being held
		}
		break;
	default:
		break;
